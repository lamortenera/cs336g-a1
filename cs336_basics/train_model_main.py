...
class Logger(object):
    def __init__(
            ...
            run_name: str, run_desc: str, config: dict, use_wandb: bool = True):
        print("Use wandb: ", use_wandb)
        self.use_wandb = use_wandb
        if self.use_wandb:
            wandb.login()
        self.init_args = {"project": "cs336_train_lm",
                          "name": run_name, "notes": run_desc, "config": config}
...
def resource_estimate(args):
    ...
    num_params += args.num_layers * (
        args.d_model*args.d_model*4 + 2*args.d_model + 3*args.d_model*args.d_ff)
    ...
    print(
        f"Estimated activations: {num_activations}, ({4*num_activations} bytes)")
    print(
        f"Estimated total: {num_activations + 4*num_params}, ({4*(num_activations + 4*num_params)} bytes)")
...
def train(args, run_name, logger):
    ...
    train_tokens = lm_training.TokenLoader(
        input_path=args.train_path, batch_size=args.batch_size,
        context_length=args.context_length, device=device)
    eval_tokens = lm_training.TokenLoader(
        input_path=args.eval_path, batch_size=args.batch_size,
        context_length=args.context_length, device=device)
    ...
    model = transformer.TransformerLM(
        ...
        max_seq_len=args.context_length, theta=theta, dtype=dtype).to(
        args.device)
    if os.name != "nt":
    ...
    optimizer = lm_training.AdamW(
        model.parameters(),
        lr_schedule_spec=args.lr_schedule, weight_decay=args.weight_decay,
    ...
    for step in range(1, args.train_steps+1):
        ...
        if step % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                def get_validation_loss():
                    ...
                    return lm_training.cross_entropy(
                        eval_outputs, eval_labels).item()
        ...
        if step % args.checkpoint_interval == 0:
            out_path = args.checkpoint_dir + "/" + \
                run_name + f"/checkpoint_{step}/data.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run_id_prefix",
        help="A valid python identifier to identify this run", required=True)
    parser.add_argument(
        "--run_desc", help="A description for this run", required=True)
    parser.add_argument(
        "--train_path",
        help="Path to a numpy array of uint16 with the tokens, serialized as bytes",
        required=True)
    parser.add_argument(
        "--eval_path",
        help="Path to a numpy array of uint16 with the tokens, serialized as bytes",
        required=True)
    parser.add_argument(
        "--train_steps", help="How many training steps to run in total",
        type=int, default=5000)
    parser.add_argument(
        "--eval_steps",
        help="How many eval steps to run every time that there is an eval phase",
        type=int, default=25)
    parser.add_argument(
        "--eval_interval",
    ...
    parser.add_argument(
        "--checkpoint_dir", help="Directory where to save checkpoints",
        required=True)
    parser.add_argument(
        "--logs_path",
        help="Path to an output file with a json dict per line, if not provided, no logs will be written other than wandb and stdout")
    parser.add_argument(
        "--use_wandb", help="If true, log data to weight and biases",
        default="True")
    parser.add_argument(
        "--vocab_size",
        help="Total number of tokens, it will be assumed the largest token is vocab_size -1",
        type=int, required=True)
    parser.add_argument("--endoftext_token",
                        help="The token that represents the end of text",
                        type=int, default=0)
    ...
    parser.add_argument(
        "--lr_schedule",
        help="Learning rate schedule in the format '0,lr0|fn0|x1,y1|fn1|...|xn,yn', where fni can be LIN or COS",
        required=True)
    parser.add_argument("--beta1", help="Beta1 param in AdamW",
                        type=float, default=0.9)
    parser.add_argument("--beta2", help="Beta2 param in AdamW",
                        type=float, default=0.999)
    parser.add_argument(
        "--weight_decay", help="Weight decay parameter in AdamW", type=float,
        default=0.01)
    ...
    parser.add_argument(
        "--gradient_clipping_l2norm",
        help="L2 norm for gradient clipping, if not provided, no clipping",
        type=float)
    ...
    logger = Logger(
        args.logs_path, run_name, args.run_desc, args_dict,
        use_wandb=parse_explicit_bool(args.use_wandb))
