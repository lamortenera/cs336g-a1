...
def get_weights(shape, init_std, device, dtype):
    return torch.nn.Parameter(
        torch.nn.init.trunc_normal_(
            ...
            std=init_std, a=-3 * init_std, b=3 * init_std))
...
class RotaryPositionalEmbedding(torch.nn.Module):
    ...
    def forward(self,
        ...
        if type(token_positions) == int:
            ...
            rotation = self.rotation[token_positions.flatten()].reshape(
                list(token_positions.shape) + [self.rotation.shape[-1]])
...
def softmax(tensor: torch.Tensor, dim_index: int) -> torch.Tensor:
    norm_exp = torch.exp(
        tensor - torch.max(tensor, axis=dim_index, keepdims=True).values)
    return norm_exp / torch.sum(norm_exp, axis=dim_index, keepdims=True)


def scaled_dot_product_attention(queries: Float
                                 [torch.Tensor, "... num_q d_k"],
                                 keys: Float[torch.Tensor, "... num_k d_k"],
                                 values: Float[torch.Tensor, "... num_k d_v"],
                                 mask: Bool[torch.Tensor, "num_q num_k"] |
    ...
    return einsum(
        torch.softmax(similarities, -1),
        values, "... num_q num_k, ... num_k d_v -> ... num_q d_v")
...
class MultiHeadSelfAttention(torch.nn.Module):
    def __init__(
        ...
        self.weights_qkv = get_weights(
            (3 * d_model, d_model),
            np.sqrt(2 / (d_model + d_model)),
            device=device, dtype=dtype)
        self.weights_o = get_weights((d_model, d_model), np.sqrt(
            2/(d_model + d_model)), device=device, dtype=dtype)
    ...
    def forward(
        ...
        O = rearrange(
            O, "num_heads ... seq_len d_k -> ... seq_len (num_heads d_k)")
        result = einsum(
            O, self.weights_o,
...
class TransformerBlock(torch.nn.Module):
    def __init__(self, d_model: int, num_heads: int, d_ff: int,
        ...
        self.mha = MultiHeadSelfAttention(
            d_model, num_heads, max_seq_len, theta, device=device,
...
def get_attention_mask(tokens: Int[torch.Tensor, "... seq_len"],
                       eos_token: Optional[int] = 0) -> Bool[torch.Tensor,
                                                             "... seq_len seq_len"]:
    ...
    min_token_for_attention = torch.cummax(
...
class TransformerLM(torch.nn.Module):
    def __init__(
            self, *, vocab_size: int, num_layers: int, d_model: int,
            num_heads: int, d_ff: int, max_seq_len: int, theta: Optional
            [float],
            eos_token: int = 0, device=None, dtype=None, attention_dtype=None):
        super().__init__()
    ...
    def forward(self, indices: Int[torch.Tensor, "... seq_len"]
